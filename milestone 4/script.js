...
createApp({
  ...
  methods: {
    ...
    toggleEmojiPicker() {
      // inverte lo stato del picker delle emoji
      this.showEmojiPicker = !this.showEmojiPicker;

      // Scroll automatico verso il basso all'apetura del picker
      setTimeout(() => {
      this.$refs.chatContainer.scrollTop = this.$refs.chatContainer.scrollHeight;
        console.log('a capo lento');
      }, 10);
    ...
    insertEmoji(emoji) {
      // inserisce l'emoji nell'input del messaggio
      this.writtenMessage += emoji;
